...
const allowedOrigins = [process.env.FRONTEND_URL,'https://grocer-ease-tawny.vercel.app', 'http://localhost:5173', 'http://localhost:5174', 'http://localhost:5175'];
...
io.on('connection', (socket) => {
    ...
    socket.on('driver:location:update', async (data) => {
        ...
        await redis.set(`driver_loc:${driverId}`, JSON.stringify({ lat, lon, heading, speed, updatedAt: new Date() }), 'EX', 60)
    ...
    socket.on('user:location:update', async (data) => {
        ...
        // Store in Redis
        await redis.set(`user_loc:${userId}`, JSON.stringify({ lat, lon, updatedAt: new Date() }), 'EX', 60)
    })
