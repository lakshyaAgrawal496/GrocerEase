  ...
  return (
    <div>
        ...
        <div className='text-sm flex items-center gap-2'>
          <span className='max-w-52 font-medium text-ellipsis line-clamp-1'>{user.name || user.mobile} <span className='text-medium text-red-600 font-medium'>{user.role === "ADMIN" ? "(Admin)" : "" }</span></span>
        ...
        <div className='text-sm grid gap-1 font-medium'>
            {
              isAdmin(user.role) && (
                <Link onClick={handleClose} to={"/dashboard/admin-dashboard"} className='px-2 hover:bg-orange-200 py-1 font-semibold text-orange-600'>Dashboard</Link>
              )
            }
            {
              isAdmin(user.role) && (
                <Link onClick={handleClose} to={"/dashboard/orders"} className='px-2 hover:bg-orange-200 py-1 font-semibold text-orange-600'>Orders</Link>
              )
            }
            {
              isAdmin(user.role) && (
